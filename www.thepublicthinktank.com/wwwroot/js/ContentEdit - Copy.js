...
function setEditSolutionContent(container) {


    const solutionId = container.getAttribute("data-content-id")
    const solutionCard = document.querySelector(`*[id="${solutionId}"]`)

    // Fetch solution editor from server
    fetch(`/edit-solution?solutionId=${solutionId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`Server returned ${response.status}: ${response.statusText}`);
            }
        ...
        .then(html => {
            ...
            tempContainer.innerHTML = html.content;
            const editIssueTemplate = tempContainer.firstElementChild;
            solutionCard.replaceWith(editIssueTemplate);
            // Initialize any new editors that were just added
            initListenersOnFormElements(editIssueTemplate);

        })
        .catch(error => {
            console.error("Error fetching solution editor:", error);
        });
}

function initListenersOnFormElements(node) {
...
function handleEditButtonAdded(node) {
    if (node.classList.contains("edit-issue-button")) {
    ...
    if (node.classList.contains("edit-solution-button")) {
        // Initialize card-specific JS here
        if (typeof setEditSolutionContent === 'function') {
            try {
                node.addEventListener("click", () => setEditSolutionContent(node));
            } catch (initError) {
                console.error("Error in setEditSolutionContent:", initError);
            }
        } else {
            console.error("setEditSolutionContent function is not defined");
        }
    }
