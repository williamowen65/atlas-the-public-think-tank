...
| Reaction definition | Graph | Curated catalog | `data/reaction-definitions.json` | Node-reaction selection and accessible presentation |
| Node-reaction association | Graph | Graph | `data/node-reactions.json` | Voting by `NodeReactionId`; presentation |
...
| Vote summary | Voting | Voting derives it from current vote records | Computed on read | Node views and future NodeReaction presentation by target ID |
...
Voting references participant and target identifiers without owning the corresponding Participant, Node, NodeReaction, or ReactionDefinition. See the [Voting boundary](VOTING.md) for policy and open decisions.\n\n## Questions to resolve before service extraction
